...
use crate::workflow::{StreamlineDataset, StreamlineFlow, TractographyPlan};
...
pub fn run_gpu_tractography(
    ...
    // B matrix: (n_dirs, ncoeffs) row-major from the sample plan.
    ...
    // Dense LUT: maps (i,j,k) → compact index or 0xFFFF_FFFF.
    ...
    let step_size_vox = plan.step_size_mm
        / vox_to_ras
            .col(0)
            .truncate()
            .length()
            .max(vox_to_ras.col(1).truncate().length())
            .max(vox_to_ras.col(2).truncate().length());
    let max_angle_cos = plan.max_angle_deg.to_radians().cos();

    // ── seeds (enumerate nonzero voxels, convert RAS → vox) ─────────────
    ...
    if total_seeds == 0 {
        return empty_flow(plan);
    }

    let max_pts = plan.max_points;

    eprintln!(
        "[gpu_tractography] '{}': {} seeds, {} dirs, {} coeffs",
        plan.label,
        total_seeds,
        n_dirs,
        ncoeffs,
    ...
    while batch_offset < total_seeds {
        ...
        for s in 0..batch_size as usize {
            ...
            // Convert vox → RAS.
            let start_offset = all_positions.len() as u32;
            for pt_vox in &streamline {
                let ras = vox_to_ras.transform_point3(glam::Vec3::from_array(*pt_vox));
                all_positions.push(ras.to_array());
            }
            all_offsets.push(start_offset + streamline.len() as u32);
        }
        ...
        if batch_offset % (BATCH_SIZE * 4) == 0 && batch_offset > 0 {
            ...
            eprintln!(
                "[gpu_tractography] {}/{} seeds ({:.0}/s) {} streamlines so far",
                batch_offset,
                total_seeds,
                rate,
                all_offsets.len() - 1,
    ...
    eprintln!(
        "[gpu_tractography] '{}': done in {:.1}s — {} streamlines",
        plan.label,
...
fn empty_flow(plan: &TractographyPlan) -> WorkflowResult<StreamlineFlow> {
