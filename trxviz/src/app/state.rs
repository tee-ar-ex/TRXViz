...
pub struct WorkflowState {
    ...
    /// Per-draw GPU upload fingerprints, keyed by `UploadSlot`. Generic
    /// replacement for the per-draw-type `uploaded_*` fields: voxel-mask
    /// meshes use it (slot kind `"voxel_mask"`, id = draw_id) so the
    /// per-frame loop skips `set_bundle_meshes` when a cached mesh is
    /// unchanged — pyAFQ Plan exposes 4–6 at once and re-uploading every
    /// frame stalls the viewport. New drawing ops reuse this instead of
    /// adding a field here.
    pub upload_cache: trxviz_core::workflow::UploadCache,
...
impl WorkflowState {
    pub fn new(
        ...
        Self {
            ...
            upload_cache: trxviz_core::workflow::UploadCache::default(),
