...
/// exist. Anything else passes through unchanged. Colors are generated by
/// the shared golden-angle HSV palette so adding more masks past the old
/// 12-entry hand-tuned list keeps producing visually distinct ROIs.
    ...
    match op {
        WorkflowNodeKind::VoxelMaskDisplay {
            ...
            let existing = snarl
                ...
                .count();
            let color = trxviz_core::palette::distinct_color_index(existing);
            WorkflowNodeKind::VoxelMaskDisplay {
                color,
                opacity,
                smooth_sigma,
